...
- Приём логов по **syslog** (TCP/UDP, порт 514) с прямой передачей в backend; syslog-ng и stats-collector — **опциональные** compose-профили
- Ручная загрузка логов через веб-интерфейс
- Парсинг **UserGate, FortiGate, Cisco ASA, Cisco FTD (FirePower), Cowrie (honeypot)** и универсальный фолбэк (Generic KV). Парсеры Cisco работают не идеально; UserGate и FortiGate — стабильнее
