...
_ga_http_port_from_env() {
    local p
    p="$(_ga_env_get HTTP_PORT)"
    [[ -z "$p" ]] && p="80"
    echo "${HTTP_PORT:-$p}"
}

_ga_https_port_from_env() {
    local p
    p="$(_ga_env_get HTTPS_PORT)"
    [[ -z "$p" ]] && p="443"
    echo "${HTTPS_PORT:-$p}"
}

HTTP_PORT="$(_ga_http_port_from_env)"
HTTPS_PORT="$(_ga_https_port_from_env)"
HTTPS_ON=0
if ga_https_active "$SCRIPT_DIR"; then
    HTTPS_ON=1
fi
...
prepare_mounts() {
    ...
    if [[ -f "${SCRIPT_DIR}/deploy/common/install_meta.sh" ]]; then
        ...
        ga_write_install_meta "$SCRIPT_DIR"
...
# Генерирует API_AUTH_TOKEN, SESSION_SECRET, пароль admin, CLICKHOUSE_PASSWORD и INGEST_SHARED_SECRET.
# Operator не сеем — УЗ создают в UI после входа.
ensure_api_auth_token() {
    local env_file=".env"
    local need_token=1 need_session=1 need_admin=1 need_ch=1 need_ingest=1
    ...
    if [[ -f "$env_file" ]] && grep -qE '^[[:space:]]*INGEST_SHARED_SECRET=.+' "$env_file"; then
        need_ingest=0
    fi
    ...
    if (( need_token == 0 && need_session == 0 && need_admin == 0 && need_ch == 0 && need_ingest == 0 )); then
        write_ingest_auth_conf
        return 0
    fi
    local rand admin_user admin_pass ch_pass ingest_secret
    rand="$(ga_rand_hex 32)"
    ...
    if (( need_admin == 1 )); then
        ...
        export AUTH_ADMIN_USER AUTH_ADMIN_PASSWORD AUTH_ADMIN_MUST_RESET GA_ADMIN_PASSWORD_PRINT
    fi
    if (( need_ch == 1 )); then
        ch_pass="$(ga_rand_hex 32)"
    fi
    if (( need_ingest == 1 )); then
        ingest_secret="$(ga_rand_hex 32)"
    fi
    {
        echo ""
        ...
        if (( need_ingest == 1 )); then
            echo "# Сгенерировано start.sh — токен маркера syslog-ng → backend :1514"
            echo "INGEST_SHARED_SECRET=${ingest_secret}"
            echo "INGEST_ALLOW_FROM=${INGEST_ALLOW_FROM:-syslog-ng}"
        fi
    } >>"$env_file"
    log "Сгенерированы секреты в .env (API_AUTH_TOKEN / SESSION_SECRET / пароль admin / CLICKHOUSE_PASSWORD / INGEST_SHARED_SECRET, если отсутствовали)"
    write_ingest_auth_conf
}
...
write_ingest_auth_conf() {
    ...
    if [[ -n "${INGEST_SHARED_SECRET:-}" ]]; then
        ...
        secret="$(grep -E '^[[:space:]]*INGEST_SHARED_SECRET=' "$env_file" | tail -n1 | cut -d= -f2- || true)"
    fi
    if [[ -z "$secret" ]]; then
        if [[ "${GA_ALLOW_INSECURE:-0}" == "1" ]]; then
            secret=""
        else
            return 0
        fi
    ...
    mkdir -p syslog-ng.d
    # Escape for syslog-ng double-quoted @define value.
    local esc="${secret//\\/\\\\}"
    esc="${esc//\"/\\\"}"
    cat >syslog-ng.d/zz_ingest_auth.conf <<EOF
# Generated by start.sh — do not commit (see .gitignore).
@define ga_ingest_token "${esc}"
EOF
}

# Не даём поднять стек без секретов / с legacy-плейсхолдерами.
# Обход только через GA_ALLOW_INSECURE=1 (local/dev).
check_auth_secrets() {
    local env_file=".env"
    local allow="${GA_ALLOW_INSECURE:-0}"
    local token="" secret="" ingest=""
    local admin_pass="" operator_pass=""

    if [[ -f "$env_file" ]]; then
        allow="$(grep -E '^[[:space:]]*GA_ALLOW_INSECURE=' "$env_file" 2>/dev/null | tail -n1 | cut -d= -f2- || true)"
        [[ -z "$allow" ]] && allow="${GA_ALLOW_INSECURE:-0}"
        ...
        ingest="$(grep -E '^[[:space:]]*INGEST_SHARED_SECRET=' "$env_file" | tail -n1 | cut -d= -f2- || true)"
    ...
    [[ -n "${GA_ALLOW_INSECURE:-}" ]] && allow="$GA_ALLOW_INSECURE"
    [[ -n "${API_AUTH_TOKEN:-}" ]] && token="$API_AUTH_TOKEN"
    [[ -n "${SESSION_SECRET:-}" ]] && secret="$SESSION_SECRET"
    [[ -n "${INGEST_SHARED_SECRET:-}" ]] && ingest="$INGEST_SHARED_SECRET"
    ...
    if [[ "$allow" == "1" || "$allow" == "true" || "$allow" == "yes" ]]; then
        log "ВНИМАНИЕ: GA_ALLOW_INSECURE=$allow — небезопасные плейсхолдеры разрешены (только для разработки)"
        return 0
    fi

    if [[ -z "$token" || -z "$secret" || -z "$ingest" ]]; then
        log "ОШИБКА: в .env обязательны API_AUTH_TOKEN, SESSION_SECRET и INGEST_SHARED_SECRET."
        log "  Запустите через ./start.sh (сгенерирует секреты) или задайте вручную."
        log "  Только для локальной разработки: GA_ALLOW_INSECURE=1 ./start.sh"
        exit 1
    fi
