...
`ByteSurgeFloor` и `BeaconMinRegularity` (0.55–0.6) задают нижнюю планку шума и минимальную «ровность» часовых интервалов; в env не выносятся — только через профиль или UI overrides.

### Редактирование порогов (UI)

Администратор на **`/anomalies/engine`** может переопределить пороги детекторов (`GET`/`PUT /api/anomalies/settings`). Значения пишутся в `/app/data/anomaly_settings.json` и подхватываются без рестарта; сброс возвращает defaults профиля. Подписи и группировка полей — в UI (help по каждому порогу).

### Окно обучения

Пока `learning=true` (возраст данных < `ANOMALY_LEARNING_DAYS` / `learning_days` в settings), детекторы **`new_country_dst`** и **`beaconing`** пропускаются (нет ложных «новых» стран и периодики на холодном старте). Остальные детекторы работают.
