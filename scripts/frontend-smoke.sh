...
[[ -f frontend/public/fonts/Geist-Variable.woff2 ]] || fail "missing Geist-Variable.woff2"
[[ -f frontend/public/fonts/GeistMono-Variable.woff2 ]] || fail "missing GeistMono-Variable.woff2"
[[ -f frontend/public/fonts/OFL.txt ]] || fail "missing fonts/OFL.txt"
...
grep -q 'scripts/check-module-identity.sh' .github/workflows/ci.yml || fail "ci: module identity"
...
grep -q 'ga_csrf' frontend/src/api/client.ts || fail "client.ts: CSRF cookie"
...
grep -q 'ROLE_DASHBOARD' frontend/src/api/types.ts || fail "types: ROLE_DASHBOARD"
grep -q 'USER_ROLE_OPTIONS' frontend/src/pages/Users/UsersPage.tsx || fail "UsersPage: USER_ROLE_OPTIONS"
grep -q 'AllRoles' backend/internal/auth/roles.go || fail "auth: AllRoles SoT"
...
grep -q 'ga-meta-version\|/api/system/version' frontend/src/components/Shell.tsx || fail "version in user menu"
grep -q 'safeNext' frontend/src/lib/format.ts || fail "safeNext"
...
grep -q 'ga-theme-change' frontend/src/pages/System/SystemPage.tsx || fail "system: theme listener"
...
[[ -f frontend/knip.json ]] || fail "missing knip.json"
