...
ver="$(ga_package_read_version "$pkg" || true)"
[[ -n "$ver" ]] || fail "не прочитали VERSION из tar.gz"
ok "package version $ver"

dest="$(mktemp -d "${TMPDIR:-/tmp}/ga-apply-dest.XXXXXX")"
...
ga_apply_install_payload "$pkg" "$dest"
...
[[ -f "${dest}/.ga-package" ]] || fail "нет .ga-package"
...
stage="$(mktemp -d "${TMPDIR:-/tmp}/ga-apply-dir.XXXXXX")"
tar -xzf "$pkg" -C "$stage"
root="$(ga_package_find_root "$stage")"
[[ -n "$root" ]] || fail "find_root"
dest2="$(mktemp -d "${TMPDIR:-/tmp}/ga-apply-dest2.XXXXXX")"
echo 'FROMDIR=1' >"${dest2}/.env"
ga_apply_install_payload "$root" "$dest2"
grep -q 'FROMDIR=1' "${dest2}/.env" || fail "dir apply потерял .env"
[[ -f "${dest2}/start.sh" ]] || fail "dir apply нет start.sh"
ok "directory apply"

dest3="$(mktemp -d "${TMPDIR:-/tmp}/ga-apply-dest3.XXXXXX")"
export GA_INSTALL_PACKAGE="$pkg"
ga_fetch_project "$dest3"
[[ -f "${dest3}/start.sh" ]] || fail "ga_fetch_project не наложил пакет"
ok "ga_fetch_project from GA_INSTALL_PACKAGE"

# shellcheck source=deploy/common/select_source.sh
source deploy/common/select_source.sh
unset GA_INSTALL_PACKAGE BRANCH GA_INSTALL_SOURCE GA_INSTALL_IS_TAG
export GA_INSTALL_PACKAGE="$pkg"
confirm_install_source
[[ "${GA_INSTALL_SOURCE}" == "package" ]] || fail "confirm_install_source source=${GA_INSTALL_SOURCE}"
[[ -n "${BRANCH:-}" ]] || fail "confirm_install_source не выставил BRANCH"
ok "confirm_install_source package"

# Path traversal
evil="$(mktemp -d "${TMPDIR:-/tmp}/ga-evil.XXXXXX")"
mkdir -p "${evil}/safe"
...
if tar -czf "${evil}/bad.tar.gz" -C "$evil" --transform='s,safe,../outside,' safe/file 2>/dev/null; then
    if ga_package_reject_unsafe_tar "${evil}/bad.tar.gz"; then
